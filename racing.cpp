...
void drawVehicle()//car for racing
{
  ...
  glBegin(GL_QUADS);
          glColor3ub(255,180,0);//middle body
...
void display()
{
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    glColor3f(1,1,1);
    drawText("--RULES--", 170, 290);
    drawText("If moving car touches the any", 170, 250);
    drawText(" obstacles then u lose the game", 170, 210);
    drawText("Use w to accelerate", 170, 170);
    drawText("Press 2 to continue", 170, 130);
    glutSwapBuffers();
    }
    else
    {
    ...
    drawText("Time(s):", 360,470);
    itoa (score, buffer, 10);
    drawTextNum(buffer, 5, 420,455);
      drawTextNum(buffer, 2, 420,470);
    glutSwapBuffers();
    for(q = 0; q<= 10000000; q++){;}
    if(collide == true)
    {
        glColor3f(0,0,0);
        drawText("Game Over", 10,30);
        glutSwapBuffers();
       getchar();
 exit(0);
    }
